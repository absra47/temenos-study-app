...
const CONCEPTS = {
  ...
  // ============ COURSE · Temenos Functional Training (TFT) — Day 6 ============
  // ---- Section 1: Deposit Application Processing — Existing Customers ----
  tftDAOverview: {
    title: "Deposit Origination — Deal Processing Overview",
    simple: "Deposit Origination brings together Product Setup, Deposit Origination (DO) Parameters, Prospect Customer handling, and Checklist Items to run applications through EM.DO.APPLICATION's five stages: Application Input, Review/Approval, Update Customer, Offer Production, and Deposit Creation.",
    example: "The parameters described in the Origination course, plus Product Builder and other external tables, all work together for deposit origination: Product Setup provides the product limits (amount, term, interest rates), Origination parameters guide the workflow and decide which stages are required versus skippable.",
    why: "An institution may want to process applications for customers who don't exist in the system yet (prospects) before they're created as full customers — which is exactly why Prospect Customer handling sits alongside the other three inputs as a first-class part of the overview diagram, not an edge case.",
    how: "Based on Deposit Origination parameter setup, any of the five EM.DO.APPLICATION stages may be Required, Optional, or Not Required — the same Required/Optional/Not Required vocabulary already established for loan origination.",
    memory: "Application Input → Review/Approval → Update Customer → Offer Production → Deposit Creation — Update Customer only really matters when a prospect is involved.",
    temenos: "Checklist items may additionally be required for some deposit products, following the same checklist pattern used in loan origination.",
    related: ["tftDAInputExisting", "tftDAProspectOverview"],
  },
  tftDAInputExisting: {
    title: "Application Input — Existing Customers",
    simple: "Deposit Input captures Existing Customer (Yes/No), the Customer Id, and Deposit Details — a Deposit Product restricted to deposit/savings-plan products only, currency, amount, term, and interest rate — plus Maturity Instruction and Rollover behaviour.",
    example: "Deposit Input / DO260068NG5F4G: Existing Customer Yes, Customer Id 100004 Nguyen, Deposit Product 12MonthsDeposit, Currency GBP, Amount Requested 4,000, Term Requested 12M, Interest Rate 2.00, Margin Rate 1.00; Maturity Instruction Rollover, Rollover Type Principal And Interest, Int. Pay Method Capitalise.",
    why: "The 'Existing Customer' field only even appears if the deposit origination parameters table is configured to permit prospect-customer applications — on a straightforward, existing-customers-only setup, the field is never shown at all.",
    how: "Deposit Product dropdown shows only deposit and savings-plan products; selecting one displays currency and, if the product has one, a default amount, term and interest rate — or the user enters amount and term and the system displays the resulting rate. Maturity Instruction defaults to 'Rollover' (most deposit products are configured for automatic rollover); 'Close' is the exceptional selection when the customer requests closure at term end. Rollover Type (only relevant if Rollover is chosen) is Principal only, or Principal and Interest.",
    memory: "Existing Customer field only shows up if the parameters table allows prospect applications at all. Maturity Instruction defaults Rollover; Close is the exception.",
    temenos: "Source of Funds is a free-text field for the inputter to record additional detail about where the deposit money is coming from.",
    related: ["tftDAOverview", "tftDAChecklistStatus", "tftDATDvsSP"],
  },
  tftDAChecklistStatus: {
    title: "Checklist & Application Status — Deposit Input",
    simple: "The Checklist tab is dynamically displayed only if the deposit product is configured to require documents based on the deposit's value (or shown always, if the checklist items are always required); Application Status carries the same status-driven hold/continue logic used throughout origination.",
    example: "Checklist Tab: Item 1.1 'Positive Identification', Status dropdown offering No / Not Required / Override / Yes. Application Status: Status 101 'Deposit application input complete' moves the process forward; a status like 'Waiting for Information' instead puts it on hold.",
    why: "Making the Checklist tab conditional on the actual deposit amount (rather than always shown) means small, routine deposits don't force the inputter through documentation checks that were only ever meant for larger, higher-risk amounts.",
    how: "Clicking the Printer icon on a checklist item lets the user scan supporting documents for that item, stored against the deposit application record. Status code behaviour (which statuses hold, which continue) is parameterised on EM.DO.APPLICATION.STATUS. Notes may be entered alongside any selected status to record the reason.",
    memory: "Checklist tab: shown only if configured for this product's value, or always if the items are marked always-required. Status behaviour: fully data-driven via EM.DO.APPLICATION.STATUS.",
    temenos: "For details on how documents are actually scanned and stored, refer to the Image Management user guide.",
    related: ["tftDAInputExisting", "tftDAReviewApproval"],
  },
  tftDAReviewApproval: {
    title: "Review/Approval Stage — Deposit Applications",
    simple: "Unlike other origination stages, where a default status is applied automatically if the user doesn't pick one, Review/Approval always requires the user to actively pull down and select a status — nothing defaults here.",
    example: "Review / Approval / DO260068NG5F4G: Status options include 201 Approved, 380 Applied for in error, 460 Client Withdrawal, 480 Delegate, 971 Expired before approval; Applied Amount 4,000 / Approved Amount 4,000, Applied Term 12M / Approved Term 12M, Decision By 203 'Application Approvers'.",
    why: "Forcing a deliberate status choice at exactly this stage — the one that actually commits the bank to the deposit — guards against an approval slipping through by accident, the same logic behind Review/Approval never defaulting a status in loan origination either.",
    how: "The approver can change the Approved Amount, Term, or Interest Rate from what was applied for — but only within whatever negotiation rules the deposit product allows; a non-negotiable value is simply no-input to the approver. Before deciding, the approver can review the full application on the 'Deposit Details' tab and request the inputter for further details if necessary. Decision By is multi-valued to capture the approving officer's account officer code.",
    memory: "Review/Approval never defaults a status — always a deliberate choice. Approved Amount/Term/Rate can only change within the product's own negotiation rules.",
    temenos: "When application processing moves to another owner, the previous owner is shown as a notification to the new one — the same ownership-handoff pattern used in loan origination.",
    related: ["tftDAChecklistStatus", "tftDAOfferProduction"],
  },
  tftDAOfferProduction: {
    title: "Offer Production — Deposit Applications",
    simple: "Offer Production captures the Funding Start date and the Funding/Payout accounts, then produces whatever documents the deposit origination parameter table configures — provided the Main Holder field is set to Yes.",
    example: "Offer Production / DO260068NG5F4G: Funding Start 06 JAN 2026, Maturity/Rollover Date 06 JAN 2027; Funding Mode Account (Funding Account 1000000986 – Current, Indiv), Paying Mode Account (Pay-out Account 1000000986); Document Production: For 1 (100004 Nguyen, Main Holder), Produce Document 1.1 'Deposit Application' Yes.",
    why: "Funding through 'Account' presents an account-selection field for whichever of the customer's own accounts will fund/receive the deposit, while selecting 'Cash' hides that field entirely — the screen only ever asks for exactly the information the chosen funding path actually needs.",
    how: "Document samples are provided in the Financial Inclusion Model Bank; which documents to produce may be selected right in the deposit origination parameter table, and if so configured, they'll appear here automatically and be produced by committing this stage. Previous stages ('Deposit Details' and 'Review/Approval') can still be reviewed via their own tabs from this screen.",
    memory: "Main Holder field must be Yes for that party's documents to actually produce. Account funding mode shows an account field; Cash mode doesn't.",
    temenos: "Produced documents are stored in the customer's document repository, viewable from the Documents tab of Single Customer View.",
    related: ["tftDAReviewApproval", "tftDACreationFunding"],
  },
  tftDACreationFunding: {
    title: "Deposit Creation — Auto vs Manual Funding",
    simple: "Deposit Creation is origination's final stage: if the deposit product was set up for automatic funding, committing this stage both creates and funds the deposit in one action; otherwise the deposit is created 'Not Funded' and must be manually funded afterward, in cash, by cheque, or by account transfer.",
    example: "Deposit Creation / DO260068NG5F4G: Customer ID 100004 Nguyen, Deposit Product 12MonthsDeposit, Funding Start 06 JAN 2026, Status 901 'Create deposit'. A manually-funded deposit instead shows in the Deposits tab of SCV with status 'Not Funded', Commitment 7,500.00, Principal 0.00.",
    why: "Showing an unfunded deposit's full Commitment amount (rather than 0.00 everywhere) while Principal stays at 0.00 lets a user see at a glance both what was approved and how much of it has actually landed.",
    how: "Once created, the deposit is viewable from the Deposits tab of SCV, and its Arrangement Overview screen can be opened from there — the deposit is also listed in the Deposits tab of Role Based Home Pages. For manual funding, clicking the Operations icon on the unfunded deposit presents the funding options (Funding by Cash / Funding from Account); whichever is chosen credits the deposit account, debiting either the Teller's cash account or the customer's own funding account respectively. For the cash option specifically, all necessary fields auto-populate and the user only needs to commit — the deposit's SCV status then changes from 'Not Funded' to 'Current.'",
    memory: "Auto-funding product: Deposit Creation commit = created AND funded. Manual: created 'Not Funded' first, then funded separately via cash/account transfer/cheque.",
    temenos: "This mirrors the exact Automatic vs Manual disbursement pattern already established for loans — created-and-funded together, or created first and funded as a distinct later action.",
    related: ["tftDAOfferProduction", "tftDMFunding"],
  },

  // ---- Section 2: Prospect Customer Deposits & Savings Plans ----
  tftDAProspectOverview: {
    title: "Deposit Applications for Prospect Customers — Overview",
    simple: "When a prospect (not-yet-a-customer) applies to deposit funds, the institution may want to process the application to determine approval first — a temporary prospect customer record, holding only mandatory and other necessary fields, is created so the application can actually be processed.",
    example: "If the application is approved, the prospect record converts to a full customer record the moment either of two things happens: the customer record is updated and validated via the Customer Update stage of deposit origination, or the deposit account is actually created.",
    why: "Deleting a declined prospect's record after a configurable retention period (rather than keeping it forever, or deleting it immediately) balances two needs: giving the bank a reasonable window to reconsider or re-approach a declined applicant, without indefinitely accumulating dead prospect records.",
    how: "If the deposit application is declined, the prospect customer record is deleted after the number of days set in the Customer Retention field of the Company record.",
    memory: "Prospect → full customer conversion happens at whichever comes first: successful Customer Update, or the deposit account actually being created. Declined → deleted after the Company record's Customer Retention days.",
    temenos: "This mirrors the same prospect-to-full-customer pattern already established for loan applications — a temporary record first, converted only on approval.",
    related: ["tftDAOverview", "tftDAProspectCreation"],
  },
  tftDAProspectCreation: {
    title: "Application Input & Prospect Customer Creation",
    simple: "Setting Existing Customer to No on Deposit Input reveals a Customer Type field (Individual/Non-Individual) and a 'Create Prospect Customer' link, which opens a minimal prospect record — capturing only the fields needed to process the application — with an auto-assigned Prospect Customer ID.",
    example: "Prospect record 100384: Customer Type Individual, GB Surname Walters, GB First Name Juana, Title Miss, Date of Birth 10 MAR 1993, Gender Female, Resident Y/N Yes, Nationality US, Residence US, ID Type Drivers Licence, Expiry Date 23 JUL 2025. Address/Classification tabs then capture GB Physical Address, Industry (4000 Services), Sector (4003 Medical), Account Officer (40, Isa Omba).",
    why: "Customer Type on the prospect path matters because it determines which prospect-customer version is actually served to the user — an Individual prospect and a Non-Individual (business) prospect need genuinely different fields captured.",
    how: "Prospect Customer ID auto-assigns the moment the prospect main screen opens. Once the prospect record is committed, its Customer Id becomes selectable from the drop-down back on Deposit Input, and origination continues exactly as normal from there.",
    memory: "Create Prospect Customer link only appears when Existing Customer = No. Prospect Customer ID auto-assigns on screen open, not on commit.",
    temenos: "The Address and Classification tabs on the prospect record capture the same kind of information (address, industry/sector, account officer, loans-written-off flags) that a full customer record would eventually hold.",
    related: ["tftDAProspectOverview", "tftDACustomerUpdate"],
  },
  tftDACustomerUpdate: {
    title: "Customer Update Stage — Prospect Customers",
    simple: "Upon deposit approval, the Update Customer stage presents two links: 'Activate Customer' (opens the full customer version to complete remaining details, converting the prospect to full-customer status on commit) and 'Create Account' (presents account products so any accounts the new customer needs — e.g. to fund the deposit — can be opened).",
    example: "Customer Update / DO1928067M3MF9: Customer Id 100384 Walters, Deposit Product 3YearsTD; Activate Customer and Create Account links available; shortcuts panel offers New Current/Savings/SSIA/Special Share/Regular Share/Drawdown/Obligation Account. Application Status 401 'Customer input completed'.",
    why: "Splitting these into two separate, deliberate links — rather than one automatic conversion — gives the bank the chance to open exactly the accounts this new customer actually needs, rather than assuming a one-size-fits-all account setup for every converted prospect.",
    how: "If a newly-created account will be used to fund the deposit, the customer is directed to the Teller to deposit funds, and the deposit can then be created; alternatively, if the product allows it, the deposit can be created first and manually funded afterward, as desired.",
    memory: "Activate Customer = prospect → full customer. Create Account = open whatever accounts (AC) this new customer needs — note this link is specifically for AC accounts; AA accounts (like loans) are originated by exiting this process and returning here to continue.",
    temenos: "Committing Activate Customer is what actually flips the customer record from provisional/prospect to fully active status.",
    related: ["tftDAProspectCreation", "tftDAOfferProduction"],
  },
  tftDATDvsSP: {
    title: "Differences Between Term Deposits & Savings Plans",
    simple: "Term Deposit (TD) and Savings Plan (SP) applications share almost the same processing, with three differences: Amount Requested (TD = one fixed lump sum; SP = the regular amount deposited at agreed frequency, e.g. monthly or weekly), Maturity Instruction (TD can Close or Rollover; SP is Close only — rollover isn't a Savings Plan feature), and Int. Pay Method (TD allows either Capitalise or Pay; SP technically allows Pay too, but Capitalise is by far the most common setting, so origination defaults to it).",
    example: "Application Input Stage – Savings Plans: Deposit Product MonthlyPlan, Amount Requested 450 (the regular monthly deposit, not a lump sum), Deposit Frequency Monthly, Maturity Instruction Close, Int. Pay Method Capitalise, Int. Pay Frequency Monthly.",
    why: "Maturity Instruction being Close-only for Savings Plans reflects the product's actual purpose — a savings goal being built up toward a target date, rather than a lump sum that might sensibly roll into a fresh term.",
    how: "Application Input, Review/Approval, Customer Update (if required), Offer Production and Deposit Creation are all otherwise done exactly the same way for both product types — these three fields are the only meaningful divergence.",
    memory: "Amount Requested: TD = lump sum. SP = the regular recurring amount. Maturity Instruction: TD can roll over, SP can't. Int. Pay Method: both can technically Pay, but SP defaults to Capitalise.",
    temenos: "A Savings Plan's funding and savings schedule mechanics (regular deposits, late-payment ageing) are covered separately under Deposit Maintenance.",
    related: ["tftDAInputExisting", "tftDAHandsOn"],
  },

  // ---- Section 3: Deposit Application Hands-On ----
  tftDAHandsOn: {
    title: "Hands-On: Term Deposit & Savings Plan Applications",
    simple: "Practice 1 processes a USD 30,000 Term Deposit application (12MonthsDeposit, 12-month rollover, interest capitalised on renewal) end to end through funding. Practice 2 sets up a 3-year Savings Plan (3yearsSavPlan) for a customer saving USD 500 monthly toward their daughter's college costs, with interest capitalised every six months, funded from the customer's savings account.",
    example: "Practice 1 solution: Deposit Input (Existing Customer Yes, 12MonthsDeposit, Amount 30,000, Term 12M, Maturity Instruction Rollover, Rollover Type Principal And Interest, Int. Pay Method Capitalise) → Review/Approval (Approved as applied, Decision By Application Approvers) → Offer Production → Deposit Creation, then Deposit funding by Cash for 30,000.00 — the resulting Payment Schedule shows the full 30,000.00 due and outstanding at the 1-year maturity date, with 0.00 due in between (a fully capitalising, rollover product).",
    why: "Running one Term Deposit and one Savings Plan through the same end-to-end sequence back to back is what makes the Amount Requested / Maturity Instruction / Int. Pay Method differences between the two product types concrete, rather than just a bullet-point list to memorise.",
    how: "Practice 2 solution: Deposit Input (3yearsSavPlan, Amount Requested 500 — the recurring monthly amount, Term 3Y, Deposit Frequency Monthly, Maturity Instruction Close, Int. Pay Method Capitalise, Int. Pay Frequency Half-yearly) → Offer Production (Funding & Payout Account = the customer's own Savings Account) → Deposit Creation; the resulting Schedule shows Deposit Receipts of 500.00 Monthly and Interest Capitalise every 6 months, with the Payment Schedule confirming 500.00 due each month and interest (e.g. 1.36) capitalising into the outstanding balance every sixth month.",
    memory: "Practice 1 = one lump-sum Term Deposit, rolled over with capitalised interest. Practice 2 = a recurring Savings Plan funded from an existing account, interest capitalised half-yearly rather than monthly.",
    temenos: "The Deposit Application Report (Role Based Menu: Deposits > Deposit Applications) lists every application's current status and the number of days taken to process it so far, or until the deposit record was created — the same Proc. Days metric pattern used for the Loan Application Report.",
    related: ["tftDAInputExisting", "tftDATDvsSP", "tftDACreationFunding"],
  },

  // ---- Section 4: Deposit Maintenance, Withdrawal & Redemption ----
  tftDMAccess: {
    title: "Deposit Maintenance — Access",
    simple: "Deposit operations — funding, withdrawal, or early redemption — are accessible either through the Teller page or directly on the deposit record itself, within the Deposits page or Single Customer View.",
    example: "Branch Manager's Search Deposits screen lists every deposit arrangement with its Product, Rate, Term, Currency, Commitment, Principal and Status (Current, Not Funded, Pending Closure) — each row's Meatballs menu gives direct access to that specific deposit's operations.",
    why: "Offering both entry points serves the same two working styles seen elsewhere in Inclusive Banking — a Teller processing a walk-in transaction versus a back-office user working from a specific customer's or deposit's own record.",
    how: "From the deposit record itself (via its Arrangement Overview's New Activity link), the same operations — funding, withdrawal, redemption — are all one click away, without needing to go through the Teller path at all.",
    memory: "Two doorways to the exact same deposit operations: Teller page (transaction-first) or the deposit record directly (deposit-first).",
    temenos: "This is the deposit-side equivalent of the SCV-vs-Loan-Service-Agent access pattern already covered for loan origination.",
    related: ["tftDMFunding", "tftDMWithdrawal", "tftDMRedemption"],
  },
  tftDMFunding: {
    title: "Deposit Funding",
    simple: "Deposits may be funded manually through Cash, Cheque, or Account transfer if the product's Settlement conditions allow it; an unfunded deposit holds status 'Not Funded' with an EXP (Expected) bill type, viewable in the arrangement overview's Bills section, and funding can be in part or in full, per the product's own setup.",
    example: "Deposit FUNDING / FT/26006/GLT3W: Value Date 06 JAN 2026, Debit Account 1000000983 (Current, Indiv), Deposit 1000001291 (12MonthsDeposit), Amount Debited/Credited 8,000.00.",
    why: "Savings Plans in particular need this to work regularly, not just once — since a Savings Plan is funded on every scheduled deposit date throughout its whole term, not funded once like a Term Deposit and then left alone.",
    how: "For Savings Plans, late payments are aged through the overdue property class, and may attract a late-payment penalty in the form of charges, or result in a waived bonus, exactly as covered for the Savings Plan product itself in Product Builder.",
    memory: "Not Funded = the deposit's resting state before money actually arrives, tagged with an EXP bill. Full or partial funding both depend on the product's own configuration.",
    temenos: "This is the same overdue/ageing mechanism already built for Savings Plans back in the Deposit Product Builder material — late deposit funding is treated as an overdue payment, not a special case.",
    related: ["tftDACreationFunding", "tftDMWithdrawal"],
  },
  tftDMWithdrawal: {
    title: "Deposit Withdrawal",
    simple: "A customer needing funds earlier than planned can withdraw part of an existing deposit via the Arrangement Overview's New Activity link; the user enters the amount and withdrawal date, and a Simulation calculates any relevant charges before the withdrawal is actually confirmed.",
    example: "Withdrawal Simulation / AASIM26006CWZY91D2: Arrangement AA25307CZ9GR (1000000465, 5YearsDeposit), Withdrawal Date 06 JAN 2026, Withdrawal Amount 7000, Currency GBP — Simulation Status 'Completed – Successfully', Activity 'Partial Withdrawal of Deposit'.",
    why: "Running the simulation before committing lets both the bank and the customer see the exact impact — including any charges — before money actually moves, rather than discovering a fee only after the withdrawal has already gone through.",
    how: "Simulation of a redemption or partial withdrawal can be done on the current date, a future date, or even a backdated date. How often, and how much, may be withdrawn in a given period can be controlled through ACTIVITY.RESTRICTION and PERIODIC.RULES. Charges attached to a withdrawal can be made due immediately or deferred, collected for all transactions or only select ones, and a Charge API routine is available specifically for early-redemption or withdrawal penalty calculations.",
    memory: "Simulation first (any date — current/future/backdated) → charges calculated → then the actual withdrawal transaction, which itself needs a second authorisation step.",
    temenos: "The withdrawal transaction produces a Withdrawal Statement, viewable again later from the arrangement overview, and updates both the account's Financial Summary and its full Activity Log once authorised.",
    related: ["tftDMAccess", "tftDMSimulationClosure"],
  },
  tftDMSimulationClosure: {
    title: "Simulation of Closure & Settlement Update",
    simple: "Choosing 'Redeem Deposit' triggers a simulation process for the chosen closure date; a Redemption Statement is produced (viewable from the Overview screen, with an icon to complete the transaction), and only when the actual redemption is confirmed does the system present the settlement account choice via the DEPOSITS-REDEEM-ARRANGEMENT activity.",
    example: "Withdrawal Statement – Inclusive Banking MB: As on 06 JAN 2026, Arrangement, Current Balance, Withdrawal Amount, Withdrawal Date, Interest At Maturity vs New Interest At Maturity — all shown before the actual Withdrawal Deposit / AAACT26006CWZYHV39 transaction is committed.",
    why: "Separating the statement (informational, no commitment) from the actual transaction (which genuinely moves money) is what lets the customer make an informed choice — they can see exactly what redeeming early would cost them, and decide not to proceed, before anything is actually posted.",
    how: "Once the actual redemption is triggered, the settlement instructions/accounts may still be updated if required. Maturity proceeds settle as configured, and the record moves to PENDING.CLOSURE; final closure then follows whatever closure conditions were set on the product (matching the same closure mechanics covered for loans).",
    memory: "Redeem Deposit → Simulation → Redemption Statement (informational) → confirm → settlement account choice (DEPOSITS-REDEEM-ARRANGEMENT) → PENDING.CLOSURE.",
    temenos: "Pre-closure of a contract is permissible by virtue of withdrawing all available balances, including accrued interest — it's mechanically a full withdrawal, not a separate closure-specific transaction type.",
    related: ["tftDMWithdrawal", "tftDMRedemption"],
  },
  tftDMRedemption: {
    title: "Deposit Redemption",
    simple: "A customer can redeem (fully close) a deposit contract before maturity by requesting the bank close the contract and release fund balances, to a designated account or by cash; accessed from the Arrangement Overview, entering the redemption date lets Simulation calculate any charges and show the balance actually due to the customer.",
    example: "Redeem Deposit / AASIM26006HBBLHNJ: Arrangement AA25307B0VZG, Closure Reason 'Customer Requested Acct Closure', Redemption Date 06 JAN 2026 — 1000000457 5YearsDeposit; Simulation Status 'Completed – Successfully'. The follow-up Redemption Statement shows Account (27,000.00) + Deposit Interest (41.28) = Total 27,041.28.",
    why: "Once accepted, the system credits the customer with the proceeds and sets the contract to Pending Closure — subsequent COBs then actually close the account, so redemption isn't an instant, one-step account closure but a short, tracked process.",
    how: "On the redemption screen, the user clicks Validate; the system presents a final screen for the user to commit. The proceeds of the contract are then credited to the customer's designated account. Balances under Financial Summary clear once redemption completes, and the redemption statement remains accessible afterward by clicking on it again from the Arrangement Overview.",
    memory: "Redeem Deposit → enter date + Closure Reason → Validate → final commit screen → proceeds credited, status Pending Closure → COBs close the account.",
    temenos: "The Closure Reason field on redemption mirrors the same Reason for Release concept used elsewhere in the training material — always capturing why an action was taken, not just that it was.",
    related: ["tftDMSimulationClosure", "tftDMReports"],
  },
  tftDMReports: {
    title: "Deposit Reports",
    simple: "Deposit summary and detailed reports are available via Branch Manager's Deposits tab, filtered by criteria like Customer, Arrangement, Product Group, Currency, or Opening/Maturity Date — selecting a summary row and drilling down shows the detailed listing of every record matching that criteria.",
    example: "Search Deposits screen: filter fields for Customer Id, Arrangement, Product Group, Currency, Opening Date, Maturity Date (each with an equals/other operator); shortcuts panel offers Search Deposit, Pending Deposit Applications, Search Deposit Applications, Deposits Maturing in 7 Days, Deposits Maturing Next 7-14 Days, Closed/Matured Deposits.",
    why: "Having dedicated shortcuts for time-boxed views (Maturing in 7 Days, Next 7-14 Days) rather than making every user build that filter from scratch each time reflects how routinely a bank actually needs to know what's about to mature.",
    how: "After selecting a desired summary row, the user drills down on the right side of that row to show the detailed listing of records matching the criteria.",
    memory: "Search Deposits = the flexible, criteria-driven report. The shortcuts panel = pre-built, commonly-needed views (maturing soon, pending applications, closed/matured).",
    temenos: "This mirrors the Loan Application / SLA Escalation / Loan History reporting pattern already established on the lending side — flexible search plus a handful of purpose-built shortcuts.",
    related: ["tftDMRedemption", "tftDAHandsOn"],
  },

  // ---- Section 5: Deposit Maintenance Hands-On ----
  tftDMHandsOn: {
    title: "Hands-On: Funding, Withdrawal & Early Redemption",
    simple: "Practice 1 funds a previously-created, Not Funded Term Deposit in full via cash and checks the activity log/financial summary. Practice 2 partially withdraws from a previously-created, Current Term Deposit. Practice 3 processes a full early redemption (pre-closure), paying the final dues to the customer in cash.",
    example: "Practice 1 solution: GBP MonthlyPlan (1000001267), status 'Not Funded' — Funding by Cash credits the account; the Transactions tab then shows Opening Balance 0.00 → Funding by Cash 450.00 → Closing Balance 450.00 (or, in the worked example, an 'Unallocated Credit' of 450.00 shown against the arrangement, pending allocation).",
    why: "Sequencing Fund → Withdraw → Redeem across the three practices walks through a deposit's entire operational lifecycle after creation — money coming in, some of it coming back out early, and finally the whole contract being closed out ahead of schedule.",
    how: "Practice 2 and 3 solutions follow the exact same screen sequences documented in the Deposit Maintenance concept material: Practice 2 uses New Activity > Withdraw Deposit on a Current deposit, entering a partial amount and letting Simulation calculate the effect before authorising; Practice 3 uses New Activity > Redeem Deposit, entering today's date as the redemption date, validating the Redemption Statement, and completing the transaction so the final dues pay out to the customer in cash and the arrangement moves to Pending Closure.",
    memory: "Fund (Not Funded → Current) → Withdraw (partial, simulated first) → Redeem (full early closure, simulated + validated, proceeds paid out, status Pending Closure).",
    temenos: "Every step in all three practices requires override acceptance and authorisation before it actually takes effect — verify success each time via the resulting Activity Log and Financial Summary on the Arrangement Overview.",
    related: ["tftDMFunding", "tftDMWithdrawal", "tftDMRedemption"],
  },
};
...
const SCENARIOS = [
  ...
  {
    id: "scn-depositTD", courseId: "tft-day6", section: "tftD6-3", image: "scn-depositTD",
    title: "Configure: A 12-Month Term Deposit",
    source: "06-2.1 Hands-on – Deposit Application Processing, Practice 1",
    brief: "Your customer has applied to deposit USD 30,000 into your 12MonthsDeposit product, for a term of 12 months, to be rolled over at the end of the period. Interest is to be capitalized on renewal. Using the origination parameters defined for this product, process the application, view the deposit agreement, verify that the deposit has funded, and accept overrides where necessary.",
    requirements: [
      "Process the application.",
      "View the deposit agreement.",
      "Verify that the deposit has been funded.",
      "Accept overrides where necessary.",
    ],
    solution: [
      { field: "Deposit Input", value: "Existing Customer Yes, Customer Id 100044 Williams, Deposit Product 12MonthsDeposit, Currency GBP, Amount Requested 30,000, Term Requested 12M, Interest Rate 2.00" },
      { field: "Maturity/Rollover fields", value: "Maturity Instruction Rollover; Rollover Type Principal And Interest; Int. Pay Method Capitalise; Int. Pay Start Date defaulted to R_RENEWAL" },
      { field: "Review/Approval", value: "Status 201 Approved; Approval Notes 'Approved'; Applied Amount 30,000 / Approved Amount 30,000, Applied Term 12M / Approved Term 12M, Decision By 203 'Application Approvers'" },
      { field: "Offer Production", value: "Funding Mode / Paying Mode set to Cash; Produce Document 1.1 'Deposit Application' Yes; Status 701 'Generate offer documents'" },
      { field: "Deposit Creation", value: "Status 901 'Create deposit'" },
      { field: "Funding", value: "Deposit funding by Cash / TT26006VC9VP — Amount 30,000.00, Value Date 06 JAN 2026" },
      { field: "Verification — Payment Schedule", value: "Maturing in 1 Years; Total Due/Outstanding both 30,000.00 at the maturity date, with 0.00 due in the interim months — confirming a fully capitalising, rolled-over deposit" },
    ],
    memory: "Rollover Type is only meaningful because Maturity Instruction = Rollover; Int. Pay Method = Capitalise means the accrued interest folds into the balance rather than being paid out.",
    related: ["tftDAInputExisting", "tftDACreationFunding", "tftDAHandsOn"],
  },
  {
    id: "scn-savingsPlan", courseId: "tft-day6", section: "tftD6-3", image: "scn-savingsPlan",
    title: "Configure: A 3-Year College Savings Plan",
    source: "06-2.1 Hands-on – Deposit Application Processing, Practice 2",
    brief: "Your customer will send his daughter to college in 3 years' time and wishes to save USD 500 monthly so he can have enough funds to execute this plan. Your 3yearsSavPlan delivers the customer's needs. Create the customer's plan and allow for interest capitalization on a six-monthly basis. The customer's savings account will be used to fund the savings. Show the plan's schedule.",
    requirements: [
      "Create the customer's savings plan using the 3yearsSavPlan product.",
      "Allow for interest capitalization on a six-monthly basis.",
      "Fund the plan from the customer's existing savings account.",
      "Show the plan's schedule.",
    ],
    solution: [
      { field: "Deposit Input", value: "Existing Customer Yes, Customer Id 100044 Williams, Deposit Product MonthlyPlan (3yearsSavPlan), Currency GBP, Amount Requested 500 (the recurring monthly amount, not a lump sum), Term Requested 3Y" },
      { field: "Savings-Plan-specific fields", value: "Deposit Frequency Monthly; Maturity Instruction Close (no rollover, as Savings Plans don't support it); Int. Pay Method Capitalise; Int. Pay Frequency Half-yearly" },
      { field: "Offer Production", value: "Funding Mode Account, Funding Account = the customer's own Savings Account (Indiv); Pay-out Account the same" },
      { field: "Deposit Creation", value: "Status 901 'Create deposit'" },
      { field: "Verification — Schedule", value: "Deposit Receipts: Account, 500.00, Due, Monthly. Interest: Deposit Interest, Capitalise, Every 6 months. Payment Schedule confirms 500.00 due each month, with interest (e.g. 1.36) capitalising into Outstanding every sixth month" },
    ],
    memory: "Amount Requested on a Savings Plan is the recurring deposit amount, not a lump sum. Deposit Frequency (monthly) and Int. Pay Frequency (half-yearly) are two independent settings — interest doesn't have to capitalise on the same cadence as the deposits themselves.",
    related: ["tftDATDvsSP", "tftDACustomerUpdate", "tftDAHandsOn"],
  },
  {
    id: "scn-depositFund", courseId: "tft-day6", section: "tftD6-5", image: "scn-depositFund",
    title: "Fund a Not Funded Term Deposit",
    source: "06-3.1 Hands-on – Deposit Maintenance, Withdrawal and Redemption, Practice 1",
    brief: "Select a previously created Term Deposit in 'Not Funded' status, deposit cash to fund the deposit in full, approve overrides, get the record authorised, and view the activity log and financial summary of the arrangement.",
    requirements: [
      "Select a previously created Term Deposit in 'Not Funded' status.",
      "Deposit cash to fund the deposit in full.",
      "Approve overrides.",
      "Get the record authorised.",
      "View the activity log and financial summary of the arrangement.",
    ],
    solution: [
      { field: "Starting point", value: "GBP MonthlyPlan (1000001267), status 'Not Funded'" },
      { field: "Funding action", value: "Transactions Meatballs menu > Funding by Cash — Amount Debited/Credited set to the full commitment (e.g. 450.00)" },
      { field: "Overrides", value: "Accepted as prompted" },
      { field: "Authorisation", value: "Record committed and authorised" },
      { field: "Verification — Transactions", value: "Opening Balance 0.00 → Funding by cash (credit) → Closing Balance reflecting the funded amount" },
    ],
    memory: "An unfunded deposit's Commitment shows the full approved amount even while Principal sits at 0.00 — funding is what actually moves Principal up to match.",
    related: ["tftDMFunding", "tftDMAccess", "tftDMHandsOn"],
  },
  {
    id: "scn-depositWithdraw", courseId: "tft-day6", section: "tftD6-5", image: "scn-depositWithdraw",
    title: "Partially Withdraw From a Current Term Deposit",
    source: "06-3.1 Hands-on – Deposit Maintenance, Withdrawal and Redemption, Practice 2",
    brief: "Select a previously created Term Deposit in 'Current' status, withdraw part of the deposit amount, approve overrides, get the record authorised, and view the activity log and financial summary of the deposit arrangement.",
    requirements: [
      "Select a previously created Term Deposit in 'Current' status.",
      "Withdraw part of the deposit amount.",
      "Approve overrides.",
      "Get the record authorised.",
      "View the activity log and financial summary of the deposit arrangement.",
    ],
    solution: [
      { field: "New Activity path", value: "Arrangement Overview > New Activity > Withdraw Deposit" },
      { field: "Withdrawal Simulation", value: "Withdrawal Date set to today; Withdrawal Amount entered as a partial figure (e.g. 7,000 of a 27,000.00 balance); Currency defaults from the deposit" },
      { field: "Simulation result", value: "Status 'Completed – Successfully'; Activity 'Partial Withdrawal of Deposit'" },
      { field: "Overrides & authorisation", value: "Accepted and authorised — commits the actual withdrawal transaction" },
      { field: "Verification", value: "Financial Summary's Total Commitment and Principal (Current) both reduce by the withdrawn amount; Activity Log shows 'Partial Withdrawal of Deposit', 'Settle via Instructions', and 'Apply payment Activity for Pre Early Withdrawal', all Authorised" },
    ],
    memory: "Simulation always runs first (any date — current/future/backdated) so the charge impact is known before the actual withdrawal is committed.",
    related: ["tftDMWithdrawal", "tftDMSimulationClosure", "tftDMHandsOn"],
  },
  {
    id: "scn-depositRedeem", courseId: "tft-day6", section: "tftD6-5", image: "scn-depositRedeem",
    title: "Process an Early Deposit Redemption",
    source: "06-3.1 Hands-on – Deposit Maintenance, Withdrawal and Redemption, Practice 3",
    brief: "A customer wishes to redeem their deposit arrangement ahead of the maturity date. Process the deposit pre-closure and pay the final dues to the customer in cash.",
    requirements: [
      "Process the deposit pre-closure ahead of the maturity date.",
      "Pay the final dues to the customer in cash.",
    ],
    solution: [
      { field: "New Activity path", value: "Arrangement Overview > New Activity > Redeem Deposit" },
      { field: "Redemption entry", value: "Redemption Date set to today; Closure Reason 'Customer Requested Acct Closure'" },
      { field: "Simulation", value: "Status 'Completed – Successfully'; Activity 'Redeem Arrangement'" },
      { field: "Redemption Statement", value: "Account balance + Deposit Interest accrued = Total amount payable (e.g. Account 27,000.00 + Deposit Interest 41.28 = 27,041.28)" },
      { field: "Validate & commit", value: "User clicks Validate; final screen presented to commit; proceeds paid out to the customer in cash per this exercise's requirement (settlement account/mode can be amended if a different payout method is needed)" },
      { field: "Resulting status", value: "Arrangement Status 'Pending Closure'; Financial Summary balances cleared; the Redemption Statement remains accessible again from the Arrangement Overview" },
    ],
    memory: "Redeem Deposit → Simulation → Redemption Statement (informational, no commitment yet) → Validate → commit → proceeds paid out → Pending Closure → subsequent COBs actually close the account.",
    related: ["tftDMRedemption", "tftDMSimulationClosure", "tftDMHandsOn"],
  },
];
...
const SLIDE_IMG = {
  ...
  tftDAOverview: "tftDAOverview", tftDAInputExisting: "tftDAInputExisting",
  tftDAChecklistStatus: "tftDAChecklistStatus", tftDAReviewApproval: "tftDAReviewApproval",
  tftDAOfferProduction: "tftDAOfferProduction", tftDACreationFunding: "tftDACreationFunding",
  tftDAProspectCreation: "tftDAProspectCreation", tftDACustomerUpdate: "tftDACustomerUpdate",
  tftDATDvsSP: "tftDATDvsSP", tftDAHandsOn: "tftDAHandsOn",
  tftDMAccess: "tftDMAccess", tftDMFunding: "tftDMFunding", tftDMWithdrawal: "tftDMWithdrawal",
  tftDMSimulationClosure: "tftDMSimulationClosure", tftDMRedemption: "tftDMRedemption",
  tftDMReports: "tftDMReports", tftDMHandsOn: "tftDMHandsOn",
};
...
const COURSES = [
  ...
  },
  {
    id: "tft-day6",
    code: "TFT-D6",
    title: "Temenos Functional Training (TFT) — Day 6",
    description: "Deposit Application Processing for existing and prospect customers (Term Deposits & Savings Plans, with the official quiz), plus Deposit Maintenance, Withdrawal & Redemption — straight from the Sep 2026 TLC materials.",
    sections: [
      {
        id: "tftD6-1", title: "Deposit Application — Existing Customers",
        description: "The five EM.DO.APPLICATION stages for an existing customer: Application Input, Checklist/Status, Review/Approval, Offer Production, and Deposit Creation.",
        concepts: ["tftDAOverview", "tftDAInputExisting", "tftDAChecklistStatus", "tftDAReviewApproval", "tftDAOfferProduction", "tftDACreationFunding"],
        quiz: [
          q("Deposit origination runs through the application called…", ["EM.DO.PARAMETERS", "EM.DO.APPLICATION", "AA.ARR.APPLICATION", "EM.DEPOSIT.PROCESS"], 1, "Five stages: Application Input, Review/Approval, Update Customer, Offer Production, Deposit Creation."),
          q("The 'Existing Customer' field on Deposit Input appears only when…", ["the deposit amount exceeds a threshold", "the deposit origination parameters table is configured to permit prospect-customer applications", "the customer requests it", "always, on every deposit product"], 1, "On an existing-customers-only setup, the field never even shows."),
          q("Maturity Instruction on a deposit application defaults to…", ["Close", "Rollover", "no default at all", "whatever the last application used"], 1, "'Close' is the exceptional selection, chosen only when the customer requests closure at term end."),
          q("Rollover Type is only relevant when…", ["Maturity Instruction is set to Rollover", "Maturity Instruction is set to Close", "the customer is a prospect", "the deposit is a Savings Plan"], 0, "It identifies whether principal only, or principal and interest, rolls over."),
          q("The Checklist tab on Deposit Input is displayed…", ["on every deposit application, always", "only if the product requires documents based on deposit value, or always if configured as always-required", "only for prospect customers", "never — it's loan-only"], 1, "Making it conditional on value avoids forcing documentation checks on small routine deposits."),
          q("Unlike other origination stages, Review/Approval…", ["defaults a status automatically like every other stage", "never defaults a status — the user must always actively select one", "skips straight to Offer Production", "only applies to prospect customers"], 1, "Guards against an approval slipping through by accident."),
          q("Approved Amount, Term or Interest Rate can be changed by the approver…", ["freely, with no restriction", "only within whatever negotiation rules the deposit product allows", "never — these are always fixed", "only for Savings Plans"], 1, "A non-negotiable value is simply no-input to the approver."),
          q("For Offer Production documents to actually produce, the…", ["Approved Amount must exceed 1,000", "Main Holder field must be set to Yes", "customer must be a prospect", "Checklist tab must be completed"], 1, "Documents are then produced automatically by committing the Offer Production stage."),
          q("If a deposit product is set up for automatic funding, committing Deposit Creation…", ["only creates the deposit, funding is always separate", "both creates and funds the deposit in one action", "requires the Teller to intervene first", "is not possible — automatic funding doesn't exist"], 1, "Otherwise the deposit is created 'Not Funded' and must be funded manually afterward."),
        ],
      },
      {
        id: "tftD6-2", title: "Prospect Customer Deposits & Savings Plans",
        description: "Processing a deposit application for a not-yet-a-customer prospect, converting them to a full customer, and the three real differences between Term Deposits and Savings Plans.",
        concepts: ["tftDAProspectOverview", "tftDAProspectCreation", "tftDACustomerUpdate", "tftDATDvsSP"],
        quiz: [
          q("In Inclusive Banking, it is possible to originate deposit applications for prospect customers. At what point does the prospect customer become a full customer?", ["As soon as the deposit is approved", "As soon as the customer funds the deposit", "As soon as the branch manager says yes", "As soon as the Customer Update stage of deposit origination is successfully executed or as soon as an account is created for the prospect customer record"], 3, "Official trainer quiz (Deposit Application Processing, Q1)."),
          q("If a prospect's deposit application is declined, the prospect customer record is…", ["kept forever", "deleted after the number of days set in the Company record's Customer Retention field", "deleted immediately", "converted to a full customer anyway"], 1, "Balances giving the bank a window to reconsider against accumulating dead prospect records."),
          q("Setting Existing Customer to No on Deposit Input reveals…", ["a Customer Type field and a 'Create Prospect Customer' link", "the Checklist tab only", "the Offer Production stage immediately", "nothing new"], 0, "Customer Type decides which prospect-customer version (Individual vs Non-Individual) is served."),
          q("A prospect's Customer Id becomes selectable on Deposit Input…", ["only after the deposit matures", "as soon as the prospect record is committed", "only after the branch manager approves", "never — a new application must be started"], 1, "Origination then continues exactly as normal."),
          q("The Update Customer stage's 'Activate Customer' link…", ["opens the account products list", "opens the full customer version, converting the prospect to full-customer status on commit", "deletes the prospect record", "is only used for Savings Plans"], 1, "'Create Account' is the separate link for opening any needed AC accounts."),
          q("In Inclusive Banking Deposit Application, which of the following cannot be combined?", ["Maturity Instruction is CLOSE, and Rollover Type is PRINCIPAL", "Interest Pay Method is PAY, and Interest Pay Frequency is MONTHLY", "Maturity Instruction is ROLLOVER, and Interest Pay Frequency is QUARTERLY", "Interest Pay Method is CAPITALISE, and Rollover Type is PRINCIPAL and INTEREST"], 0, "Official trainer quiz (Deposit Application Processing, Q2) — Rollover Type is only ever presented/relevant when Maturity Instruction is Rollover, so it cannot pair with Close."),
          q("In Inclusive Banking, which of the following is NOT common between Term Deposits and Savings Plan?", ["Interest is capitalized at agreed frequencies", "Application may be processed for existing or prospect customers", "Deposit is closed at final maturity date", "Deposit is rolled over at agreed frequencies"], 3, "Official trainer quiz (Deposit Application Processing, Q3) — rollover is a Term Deposit feature; Savings Plans are Close-only, no rollover."),
          q("For a Savings Plan, Amount Requested on Deposit Input represents…", ["one fixed lump sum", "the amount to be deposited at agreed frequencies, e.g. monthly", "the maturity value", "the interest rate"], 1, "For a Term Deposit, by contrast, it's a single fixed amount."),
        ],
      },
      {
        id: "tftD6-3", title: "Deposit Application Hands-On",
        description: "Processing a Term Deposit and a Savings Plan application end to end, from input through funding.",
        concepts: ["tftDAHandsOn"],
        quiz: [
          q("Practice 1 processes a deposit application for…", ["USD 30,000 into a 12-month Term Deposit, rolled over with capitalised interest", "a Savings Plan only", "a prospect customer", "a group deposit"], 0, "12MonthsDeposit product, Maturity Instruction Rollover, Int. Pay Method Capitalise."),
          q("Practice 2's Savings Plan (3yearsSavPlan) is funded from…", ["cash only", "the customer's own existing savings account", "a brand-new current account", "an external guarantor's account"], 1, "Set as the Funding & Payout Account at Offer Production."),
          q("Practice 2's interest capitalises…", ["monthly, same as the deposit frequency", "every six months (half-yearly), separate from the monthly deposit frequency", "only at final maturity", "weekly"], 1, "Deposit Frequency Monthly; Int. Pay Frequency Half-yearly — two independent settings."),
          q("The Deposit Application Report shows, among other things…", ["only the customer's name", "each application's current status and the number of days taken to process it (Proc. Days)", "only the interest rate", "only the branch code"], 1, "The same Proc. Days metric pattern used for the Loan Application Report."),
        ],
      },
      {
        id: "tftD6-4", title: "Deposit Maintenance, Withdrawal & Redemption",
        description: "Funding, partial withdrawal (with simulation), and full early redemption (with closure), plus deposit reporting.",
        concepts: ["tftDMAccess", "tftDMFunding", "tftDMWithdrawal", "tftDMSimulationClosure", "tftDMRedemption", "tftDMReports"],
        quiz: [
          q("Deposit maintenance operations (funding, withdrawal, redemption) are accessible via…", ["only the Teller page", "the Teller page, or directly on the deposit record within Deposits/SCV", "only Single Customer View", "a separate maintenance module"], 1, "Two doorways to the exact same set of operations."),
          q("An unfunded deposit holds status…", ["Current", "Not Funded, with an EXP (Expected) bill type", "Pending Closure", "Written Off"], 1, "Viewable in the arrangement overview's Bills section."),
          q("Savings Plan late payments are aged through…", ["the same overdue property class as any other deposit", "a completely separate Savings Plan-only mechanism", "the Guarantor module", "manual tracking only"], 0, "May attract a late-payment penalty (charges) or a waived bonus."),
          q("Before an actual deposit withdrawal is committed, the system…", ["skips straight to posting", "runs a Simulation to calculate any relevant charges first", "requires a new application", "always rejects partial withdrawals"], 1, "Simulation can run on the current date, a future date, or even a backdated date."),
          q("How much may be withdrawn in a given period can be controlled via…", ["ACTIVITY.RESTRICTION and PERIODIC.RULES", "the Credit Scoring module", "the Guarantor Parameters table", "the CGAP Reports"], 0, "The same restriction mechanism used elsewhere in Inclusive Banking."),
          q("Choosing 'Redeem Deposit' triggers…", ["immediate account closure with no further steps", "a simulation for the chosen closure date, producing a Redemption Statement before any commitment", "a new loan application", "a guarantor recovery"], 1, "Separating the statement from the actual transaction lets the customer make an informed choice."),
          q("Once an actual deposit redemption is confirmed…", ["the account closes instantly, same day", "proceeds are credited and the contract moves to PENDING.CLOSURE, closing over subsequent COBs", "nothing happens until the customer visits a branch", "the deposit reverts to Not Funded"], 1, "Redemption is a short, tracked process, not an instant one-step closure."),
          q("Deposit reports are accessed via…", ["Branch Manager's Deposits tab, with filters like Customer, Arrangement, Product Group, Currency, Opening/Maturity Date", "only the Credit Manager module", "only SCV", "the Guarantor Reports menu"], 0, "Plus shortcuts for commonly-needed views like deposits maturing in 7 days."),
        ],
      },
      {
        id: "tftD6-5", title: "Deposit Maintenance Hands-On",
        description: "Funding a Not Funded deposit, partially withdrawing from a Current one, and processing a full early redemption.",
        concepts: ["tftDMHandsOn"],
        quiz: [
          q("Practice 1 funds a deposit that starts in which status?", ["Current", "Not Funded", "Pending Closure", "Written Off"], 1, "Funded in full via cash; overrides accepted, then authorised."),
          q("Practice 2 processes…", ["a full early redemption", "a partial withdrawal from a Current Term Deposit", "a new deposit application", "a guarantor recovery"], 1, "Uses New Activity > Withdraw Deposit, with Simulation calculating the effect first."),
          q("Practice 3's early redemption pays the final dues to the customer…", ["by account transfer only", "in cash", "by cheque only", "it doesn't pay out, it just closes the record"], 1, "Using New Activity > Redeem Deposit, validating the Redemption Statement, then completing the transaction."),
          q("After Practice 3, the arrangement's status becomes…", ["Current", "Not Funded", "Pending Closure", "unchanged"], 2, "Subsequent COBs then fully close the account."),
        ],
      },
    ],
    assessment: [
      q("Deposit origination runs through the application called…", ["EM.DO.PARAMETERS", "EM.DO.APPLICATION", "AA.ARR.APPLICATION", "EM.DEPOSIT.PROCESS"], 1, "Five stages: Application Input, Review/Approval, Update Customer, Offer Production, Deposit Creation."),
      q("Maturity Instruction on a deposit application defaults to…", ["Close", "Rollover", "no default", "whatever was last used"], 1, "'Close' is the exceptional selection."),
      q("Unlike other origination stages, Review/Approval…", ["defaults a status automatically", "never defaults a status — always a deliberate user choice", "skips straight to Offer Production", "only applies to prospects"], 1, "Guards against an accidental approval."),
      q("For Offer Production documents to actually produce, the…", ["Approved Amount must exceed 1,000", "Main Holder field must be set to Yes", "customer must be a prospect", "Checklist tab must be completed"], 1, "Documents produce automatically by committing the stage."),
      q("If a deposit product is set up for automatic funding, committing Deposit Creation…", ["only creates the deposit", "both creates and funds it in one action", "requires manual Teller intervention first", "isn't possible"], 1, "Otherwise it's created 'Not Funded' and funded separately."),
      q("At what point does a prospect customer become a full customer?", ["As soon as the deposit is approved", "As soon as the customer funds the deposit", "As soon as the branch manager says yes", "As soon as the Customer Update stage succeeds, or an account is created for the prospect"], 3, "From the official Deposit Application Processing quiz."),
      q("If a prospect's declined deposit application is declined, the prospect record is…", ["kept forever", "deleted after the days set in the Company record's Customer Retention field", "deleted immediately", "converted to full customer anyway"], 1, "Balances reconsideration time against accumulating dead records."),
      q("In Deposit Application Processing, which combination cannot occur?", ["Maturity Instruction CLOSE with Rollover Type PRINCIPAL", "Int. Pay Method PAY with Int. Pay Frequency MONTHLY", "Maturity Instruction ROLLOVER with Int. Pay Frequency QUARTERLY", "Int. Pay Method CAPITALISE with Rollover Type PRINCIPAL AND INTEREST"], 0, "From the official quiz — Rollover Type only applies when Maturity Instruction is Rollover."),
      q("Which is NOT common between Term Deposits and Savings Plans?", ["Interest capitalised at agreed frequencies", "Application processed for existing or prospect customers", "Closed at final maturity date", "Rolled over at agreed frequencies"], 3, "From the official quiz — rollover is Term-Deposit-only; Savings Plans are Close-only."),
      q("For a Savings Plan, Amount Requested represents…", ["one fixed lump sum", "the amount deposited at agreed frequencies", "the maturity value", "the interest rate"], 1, "For a Term Deposit it's instead a single fixed amount."),
      q("The Update Customer stage's 'Activate Customer' link…", ["opens the account products list", "opens the full customer version, converting the prospect on commit", "deletes the prospect record", "is Savings-Plan-only"], 1, "'Create Account' is the separate link for AC accounts."),
      q("Deposit maintenance operations are accessible via…", ["only the Teller page", "the Teller page or directly on the deposit record within Deposits/SCV", "only SCV", "a separate module"], 1, "Two doorways, same operations."),
      q("An unfunded deposit holds status…", ["Current", "Not Funded, with an EXP bill type", "Pending Closure", "Written Off"], 1, "Viewable in the arrangement overview's Bills section."),
      q("Before an actual deposit withdrawal is committed, the system…", ["skips straight to posting", "runs a Simulation to calculate charges first", "requires a new application", "always rejects partial withdrawals"], 1, "Can simulate on current, future, or backdated dates."),
      q("Choosing 'Redeem Deposit' triggers…", ["instant account closure", "a simulation producing a Redemption Statement before any commitment", "a new loan application", "a guarantor recovery"], 1, "Lets the customer make an informed choice before anything posts."),
      q("Once a deposit redemption is confirmed…", ["the account closes instantly", "proceeds credit and the contract moves to PENDING.CLOSURE, closing over subsequent COBs", "nothing happens until a branch visit", "the deposit reverts to Not Funded"], 1, "A short, tracked process, not an instant closure."),
      q("Deposit reports are accessed via…", ["Branch Manager's Deposits tab with filter criteria", "only Credit Manager", "only SCV", "the Guarantor Reports menu"], 0, "Plus shortcuts for commonly-needed views."),
      q("Savings Plan late payments are aged through…", ["the same overdue property class as any deposit", "a wholly separate mechanism", "the Guarantor module", "manual tracking only"], 0, "May attract a penalty charge or a waived bonus."),
    ],
  },
];
